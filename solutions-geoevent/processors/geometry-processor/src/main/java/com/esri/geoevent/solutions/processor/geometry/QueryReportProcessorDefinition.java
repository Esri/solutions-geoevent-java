	...
	protected void GenerateProperties() {
		try {
			...
			if (serviceConnections == null) {
				LOG.info("No service connections found:");
				return;
			}
			Iterator<ArcGISServerConnection> it = serviceConnections.iterator();
				...
				for (int i = 0; i < folders.length; ++i) {
					...
					for (int j = 0; j < fservices.length; ++j) {
						...
						PropertyDefinition pd = new PropertyDefinition(pdName,
								PropertyType.Boolean, false, "Query Service: "
										+ fs, "Set query on service", true,
								false);
						...
						for (int k = 0; k < layers.size(); ++k) {
							...
							PropertyDefinition pdwc = new PropertyDefinition(
									lyrName + "_whereclause",
									PropertyType.String, "", "-->Whereclause",
									"SQL whereclause string", false, false);
							pdwc.setDependsOn(lyrName + "=true");
							propertyDefinitions.put(pdwc.getPropertyName(),
									pdwc);

							PropertyDefinition pdCalculateDistance = new PropertyDefinition(
									lyrName + "_calcDistance",
							...
							propertyDefinitions.put(
									pdCalculateDistance.getPropertyName(),
									pdCalculateDistance);

							PropertyDefinition procDistUnits = new PropertyDefinition(
									lyrName + "_dist_units",
									PropertyType.String, 0, "Units",
							...
							procDistUnits.setDependsOn(lyrName
									+ "_calcDistance=true");
							propertyDefinitions.put(
									procDistUnits.getPropertyName(),
									procDistUnits);
							...
							PropertyDefinition pdDistanceToken = new PropertyDefinition(
									...
									false, true);
							pdDistanceToken.setDependsOn(lyrName
									+ "_calcDistance=true");
							propertyDefinitions.put(
									pdDistanceToken.getPropertyName(),
									pdDistanceToken);
							...
							for (int l = 0; l < fields.length; ++l) {
								...
								PropertyDefinition pdFld = new PropertyDefinition(
										fldPropName, PropertyType.Boolean,
										false, "---->Field: " + fld,
